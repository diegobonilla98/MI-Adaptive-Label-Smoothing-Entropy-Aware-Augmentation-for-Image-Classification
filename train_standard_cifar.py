...
class DistortedCIFARDataset(Dataset):
    def __init__(self, root_dir="./data", split="train", download=True, use_distortion=False):
        """
        CIFAR-100 Dataset with optional distortions but normal one-hot labels
    ...
    def __getitem__(self, idx):
        ...
        # Return standard one-hot encoded label (not soft labels)
        return image, label
...
def create_data_loaders(root_dir="./data", batch_size=128, num_workers=4, download=True):
    """Create train and validation data loaders for CIFAR-100"""
    
    train_dataset = DistortedCIFARDataset(root_dir, "train", download=download, use_distortion=True)
    val_dataset = DistortedCIFARDataset(root_dir, "test", download=download, use_distortion=False)
...
def train_model():
    ...
    # TensorBoard writer
    writer = SummaryWriter('runs/resnet18_cifar100_baseline')
    
    # Create checkpoints directory
    checkpoint_dir = 'runs/resnet18_cifar100_baseline/checkpoints'
    ...
    writer.close()
    print("Training completed. TensorBoard logs saved to 'runs/resnet18_cifar100_baseline'")
