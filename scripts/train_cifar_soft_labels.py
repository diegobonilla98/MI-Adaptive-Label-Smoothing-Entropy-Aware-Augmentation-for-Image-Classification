...
class DistortedCIFARDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.label_smoothing > 0.0 and self.split == "train":
            ...
            target = torch.full(
                (self.num_classes,), self.label_smoothing / self.num_classes
            )
            target[label] = (
...
def create_data_loaders(
    root_dir="./data", batch_size=128, num_workers=4, download=True, label_smoothing=0.1
):
    ...
    val_dataset = DistortedCIFARDataset(
        root_dir, "test", download=download, use_distortion=False, label_smoothing=0.0
    )
...
def train_model():
    ...
    train_loader, val_loader = create_data_loaders(
        batch_size=128, num_workers=4, label_smoothing=label_smoothing
    )
    ...
    writer.close()
    print(
        "Training completed. TensorBoard logs saved to 'runs/resnet18_cifar100_soft_labels'"
    )
    print(
