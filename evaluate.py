...
transform_test = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761))  # CIFAR-100 means/std
])
...
for name, path in MODEL_PATHS.items():
    try:
        ...
        print(f"Loaded {name}: Epoch {model_info[name]['epoch']}, Val Acc: {model_info[name]['best_val_acc']:.2f}%")
...
def apply_distortion(img, dist_type, d):
    ...
    if len(img.shape) == 4:
        ...
        return torch.stack([apply_distortion(single_img, dist_type, d) for single_img in img])
    ...
    if dist_type == 'diffusion':
        ...
        if kernel_size % 2 == 0:
            kernel_size += 1
        return transforms.functional.gaussian_blur(img, kernel_size=[kernel_size, kernel_size], sigma=sigma)
    ...
    for dist_type in dist_types:
        for level in np.linspace(0, 1, num_levels):
            ...
            with torch.no_grad():
                for data in loader:
                    images, labels = data[0].to(device), data[1].to(device)
                    degraded = torch.stack([apply_distortion(img, dist_type, level) for img in images])
...
metric_labels = ['Top-1 Accuracy (%)', 'Top-5 Accuracy (%)', 'Cross-Entropy Loss', 'Expected Calibration Error']
...
for dist_type in dist_types:
    ...
    for i, name in enumerate(models):
        ax.plot(levels, results[name]['robust_acc'][dist_type], 
               label=name, marker='o', linewidth=2.5, markersize=6, color=colors[i])
    
    ax.set_title(f'Model Robustness: Accuracy vs. {distortion_names[dist_type]} Level', 
    ...
    ax.grid(True, alpha=0.3)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, max([max(results[name]['robust_acc'][dist_type]) for name in models]) + 5)
...
for name in models:
    ...
    for i, dist_type in enumerate(dist_types):
        ax.plot(levels, results[name]['robust_acc'][dist_type], 
               label=distortion_names[dist_type], marker='s', linewidth=2.5, 
               markersize=6, color=dist_colors[i])
    ...
    plt.tight_layout()
    plt.savefig(f'robust_acc_per_dist_{name.replace(" ", "_").replace("(", "").replace(")", "").lower()}.png', 
                dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: robust_acc_per_dist_{name.replace(' ', '_').replace('(', '').replace(')', '').lower()}.png")
...
class_idx = 0
cifar100_classes = ['apple', 'aquarium_fish', 'baby', 'bear', 'beaver', 'bed', 'bee', 'beetle', 'bicycle', 'bottle']  # First 10 for reference
class_name = cifar100_classes[class_idx] if class_idx < len(cifar100_classes) else f"class_{class_idx}"
...
if class_images:
    ...
    for name, model in models.items():
        ...
        for dist_type in dist_types:
            for d in tqdm(d_steps, desc=f'{name} - {distortion_names[dist_type]}', leave=False):
                degraded = torch.stack([apply_distortion(img, dist_type, d) for img in class_images])
                with torch.no_grad():
...
if conf_results:
    for dist_type in dist_types:
        ...
        for i, name in enumerate(models):
            ax.plot(d_steps, conf_results[name][dist_type], 
                   label=name, marker='o', linewidth=2.5, markersize=6, color=colors[i])
        
        ax.set_title(f'Model Confidence vs. {distortion_names[dist_type]} Level\n(True Class: {class_name.title()})', 
                    fontsize=16, fontweight='bold')
        ax.set_xlabel('Distortion Level (0 = Original, 1 = Maximum)', fontsize=13)
        ...
        plt.tight_layout()
        plt.savefig(f'confidence_vs_{dist_type}_class{class_idx}.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    for name in models:
        ...
        ax.set_title(f'Confidence Analysis: {name} Model\n(True Class: {class_name.title()})', 
                    fontsize=16, fontweight='bold')
        ax.set_xlabel('Distortion Level (0 = Original, 1 = Maximum)', fontsize=13)
        ax.set_ylabel('Average Confidence for True Class', fontsize=13)
        ax.legend(fontsize=11, loc='best')
...
for name in models:
    print(f"\n{name}:")
    print(f"  Training Info: Epoch {model_info[name]['epoch']}, Val Acc: {model_info[name]['best_val_acc']:.2f}%")
